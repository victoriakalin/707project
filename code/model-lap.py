...
def convTrans(in_planes, out_planes, stride = 2):
  conv_t = nn.ConvTranspose2d(in_channels=in_planes, out_channels= out_planes, kernel_size=4, stride = stride , padding=1, bias=False)
  # c1, c2, h, w = conv_t.weight.data.size()
  # weight = get_upsample_filter(h)
  # conv_t.weight.data = weight.view(1, 1, h, w).repeat(c1, c2, 1, 1)
  # if conv_t.bias is not None:
  #   conv_t.bias.data.zero_()
  return conv_t


def featExtractBlock(in_planes, out_planes):
    block = nn.Sequential(
        ...
        nn.ReLU(True),
        conv3x3(in_planes, in_planes),
        convTrans(in_planes, out_planes),
        nn.ReLU(True))
    return block
...
class STAGE1_G(nn.Module):
    ...
    def define_module(self):
        ...
        # -> ngf/16 x 64 x 64
        self.feat_extract = featExtractBlock(ngf // 8, ngf//16)
...
class STAGE2_G(nn.Module):
    ...
    def define_module(self):
        ...
        # --> ngf // 4 x 256 x 256
        self.feat_extract = featExtractBlock(ngf//2, ngf//4)
